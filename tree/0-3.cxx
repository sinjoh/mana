...
void CoarsenWithMooreCursor(vtkSmartPointer<vtkUniformHyperTreeGrid> hyperTreeGrid, double coarseningThreshold) {
    ...
    for (vtkIdType leafId = 0; leafId < numLeaves; ++leafId) {
        ...
        if (CheckIfAllChildrenBelowThreshold(mooreCursor, coarseningThreshold)) {
            double parentValue = mooreCursor->GetTree()->GetCellData()->GetScalars()->GetTuple1(mooreCursor->GetGlobalNodeIndex());
            mooreCursor->CollapseLeaf();
            mooreCursor->GetTree()->GetCellData()->GetScalars()->SetTuple1(mooreCursor->GetGlobalNodeIndex(), parentValue);
        }
    }
}
...
void CoarsenWithVonNeumannCursor(vtkSmartPointer<vtkUniformHyperTreeGrid> hyperTreeGrid, double coarseningThreshold) {
    ...
    for (vtkIdType leafId = 0; leafId < numLeaves; ++leafId) {
        ...
        if (CheckIfAllChildrenBelowThreshold(vonNeumannCursor, coarseningThreshold)) {
            double parentValue = vonNeumannCursor->GetTree()->GetCellData()->GetScalars()->GetTuple1(vonNeumannCursor->GetGlobalNodeIndex());
            vonNeumannCursor->CollapseLeaf();
            vonNeumannCursor->GetTree()->GetCellData()->GetScalars()->SetTuple1(vonNeumannCursor->GetGlobalNodeIndex(), parentValue);
        }
    }
}
